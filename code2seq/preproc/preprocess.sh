#!/usr/bin/env bash
###########################################################
# Change the following values to preproc a new dataset.
...
DATASET_NAME=auto_comment_dataset
...
#EXTRACTOR_JAR=JavaExtractor/JPredict/target/JavaExtractor.jar
...
${PYTHON} JavaExtractor/extract.py --dir ${TRAIN_DIR} --max_path_length 8 --max_path_width 2 --num_threads ${NUM_THREADS} --jar ${EXTRACTOR_JAR} > ${TRAIN_DATA_FILE} 2>> error_log.txt
...
${PYTHON} preproc/preprocess.py --train_data ${TRAIN_DATA_FILE} --test_data ${TEST_DATA_FILE} --val_data ${VAL_DATA_FILE} \
...
# If all went well, the raw data files can be deleted, because preproc.py creates new files
